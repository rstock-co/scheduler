...
describe("Application", () => {
  ...
  it("loads data, cancels an interview and increases the spots remaining for Monday by 1", async () => {
    ...
    // 3. Click the "Delete" button on "Archie Cohen's" booked appointment.
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 3. Find and Click the "Edit" button on the "Archie Cohen" appointment.
    const appointment = getAllByTestId(container, "appointment").find(
      appointment => queryByText(appointment, "Archie Cohen")
    );
    fireEvent.click(queryByAltText(appointment, "Edit"));

    // 4. Change the sutdent's name and select the first interviewer
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Richard Stock" },
    });
